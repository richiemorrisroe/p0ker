...
from pkr import Game, Deck, Player, deal_cards
def test_game_is_game() -> None:
    game = Game()
    assert isinstance(game, Game)

def test_game_has_deck() -> None:
    game = Game()
    assert isinstance(game.deck, Deck)

def test_game_pot_is_zero() -> None:
    game = Game()
    assert game.pot == 0
...
def test_game_deal_cards() -> None:
    ...
    lp = [p1, p2]
    game = Game()
    original_len = len(game.deck)
    game, list_players = deal_cards(game, lp)
    p1, p2 = list_players
    assert len(game.deck) is None
...
- Discard cards needs some love:
  - i need to make some kind of stash object to hold the discarded cards
  - should probably exist as something off a Dealer/Game object
